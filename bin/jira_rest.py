...
def parse_issues(issues, fieldlist, use_internal_field_names, time_field, kv_string_fields=[]):
    ...
    for issue in issues:
        ...
        for jirafield, v in list(issue["fields"].items()):
        ...
        row = {key: value if key == '_raw' or isinstance(value, str) else json.dumps(value) for key, value in row.items()}
        row['_time'] = epoch
        row['host'] = jira_service.host
        row['source'] = 'jira_rest'
    ...
    for issues in rapidboards['issuesData']['issues']:
        ...
        for issue_id in issues:
            if 'Statistic' in issue_id:
                ...
                if 'value' in list(issues[issue_id]['statFieldValue'].keys()):
...
def run_jsqlsearch(jql, use_internal_field_names, show_comments, time_field, max_results=100000, is_issues_query=False, kv_string_fields=[]):
    offset = 0
    args = urllib.parse.quote_plus(jql).split()

    target = '/rest/api/2/search'
    fullfields = get_fields()
    #print(fullfields, file=sys.stderr)
    ...
    encoded_url_args = urllib.parse.urlencode(url_args)
...
except SearchArgException as e:
